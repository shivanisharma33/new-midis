    ...
    return (
        <section className="bg-[#0b0b0b] py-32 px-10 lg:px-20 border-t border-white/5">
            <div className="max-w-[1400px] mx-auto">
                <div className="flex flex-col lg:flex-row lg:justify-between items-start gap-20">
                    <div className="lg:sticky lg:top-40 max-w-[400px]">
                        <motion.h2
                            initial={{ opacity: 0, y: 20 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            className="text-white text-[5vw] lg:text-[4vw] font-[700] leading-[1] tracking-[-0.03em] mb-8"
                        >
                    ...
                    <div className="flex-1 w-full space-y-24 lg:space-y-32">
                        {services.map((service, index) => (
                            <motion.div
                                ...
                                className="flex flex-col md:flex-row md:items-start gap-8 md:gap-20"
                            >
                                <span className="text-white/20 text-[14px] font-bold uppercase tracking-widest pt-2">
                                    0{index + 1}
                                </span>
                                <div className="flex flex-col flex-1">
                                    <h3 className="text-white text-[32px] md:text-[48px] font-bold tracking-tight mb-8">
                                        {service.title}
                                    </h3>
                                    <div className="flex flex-wrap gap-x-6 gap-y-4">
                                        {service.items.map((item, i) => (
                                            <span
                                                key={i}
                                                className="text-white/40 text-[18px] md:text-[22px] font-medium hover:text-white transition-colors cursor-default"
                                            >
                                                {item}
                                            </span>
                                        ))}
